...
if(!defined('SCRIPTBASE'))
    define('SCRIPTBASE', $_SERVER['DOCUMENT_ROOT'] . '/root/');
...
//	 global $db;
	 ...
	 $sql = "INSERT INTO ab_admin_sections (admin_section_name, admin_section_level, admin_section_url, admin_section_parent_id, admin_section_entry_type)";
	 $sql.= " VALUES ('".$sName."',".$iLevel.",'".$sURL."',".$iParentID.",".$iContentType.")";
...
//Function Update Section does not concern with moving the section but just updating it's own paramters.
...
//		$aName = $db->fetch_array($rsTmp);
		//$sName = $aName['admin_section_name'];
                $aName = $rsTmp['admin_section_name'];
		$iNewName = 0;
	}
	else
		$iNewName = 1;
...
*/
	 ...
	 if($iContentType != $aName['admin_section_entry_type'])
	 {
		...
		$sql = "SELECT * FROM ab_admin_content_info WHERE admin_section_id=".$iSecID.";";
		//$rsTmp = $db->query($sql);
                $rsTmp = db_all($sql);
...
function move_section($iSecID = '', $iOldParentID = '', $iNewParentID = '')
{
//	global $db;
	...
	$aTarget = get_attributes($iNewParentID);
	...
	//Now, check if moving to the target is feasible
		...
		//		after the operation.
	if(($aTarget['admin_section_level']+ $iMoreLevels) > MAX_LEVELS)
	{
	...
	//Moving the section is merely changing its parent ID. 
	...
	$sql = "UPDATE ab_admin_sections SET admin_section_parent_id=".$iNewParentID." WHERE admin_section_id=".$iSecID.";";
//	$rsTmp = $db->query($sql);
        $rsTmp = db_all($sql);
	...
	if(!update_levels($iSecID, $aTarget['admin_section_level']))
	{
