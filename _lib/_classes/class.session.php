    ...
    function _authenticate($aArgs) {
        $sql = "SELECT 
                ...
                FROM 
                    ".PREFIX."_admin_users 
                WHERE 
                    admin_user_email='".$aArgs["User Name"]."' 
                    AND deleted=0";
...
//        $aRow = $rsTmp->fetchRow(DB_FETCHMODE_ASSOC);
		$aRow = $this->_oConn->fetch_array($rsTmp);

        
        // check row count
//        if ($rsTmp->numRows() < 1) {
	        if ($this->_oConn->num_rows($rsTmp) < 1) {
		...
		//Disabling Password check for development
		
		elseif (!empty($aRow["admin_user_pass"])) {
            
            // match passwords
            if (strcmp(substr(md5($aArgs["User Name"].'|'.$aArgs["Password"]), 3, 8), $aRow["admin_user_pass"])) {
