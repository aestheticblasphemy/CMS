...
//        $return = $rsTmp->numRows();
        $return = $this->_oConn->num_rows($rsTmp);
        return $return;
    }
...
*/
    ...
    function sendReminder($aArgs) {
        ...
        if ($this->_oConn->num_rows($rsTmp) > 0) {
            
            $aRow = $this->_oConn->fetch_row($rsTmp);
...
/*
        ...
        while ($aRow = $this->_oConn->fetch_row($rsTmp)) {
  ...
  */      
	...
	public function get_user_info($result)
	{
		$return =$this->_oConn->get_table_fields($result);
	//	print_r ($return);
		return ($return);
	}
